...
class TestKeyDerivationService:
    ...
    def service(self):
        ...
        return KeyDerivationService(
            memory_cost=1024, time_cost=1, parallelism=1  # Low for fast tests
        )
    ...
    def test_verify_password(self, service):
        ...
        assert (
            service.verify_password("correct_password", derived.salt, derived.key)
            is True
        )

        assert (
            service.verify_password("wrong_password", derived.salt, derived.key)
            is False
        )
...
class TestAESEncryptor:
    ...
    def test_file_encryption(self, encryptor):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            delete=False
        ) as enc, tempfile.NamedTemporaryFile(
            delete=False
        ) as dec:
            src.write(content)
            src.flush()
