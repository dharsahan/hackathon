...
class TestDeduplicationEngine:
    ...
    def test_small_file_optimization(self):
        """Test that small files don't recompute full hash."""
        from unittest.mock import patch

        engine = DeduplicationEngine(chunk_size=1024)
        content = b"Small file content"  # < 1024*3
        ...
        with tempfile.NamedTemporaryFile(
            ...
            with patch.object(
                ...
                assert result2.status == DuplicateStatus.EXACT_DUPLICATE
                # Should be 0 calls because partial hash is used as full hash for small files
                assert mock_full_hash.call_count == 0
