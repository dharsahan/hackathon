...
class TestDeduplicationEngine:
    ...
    def test_small_file_optimization(self):
        """Test that small files reuse partial hash as full hash."""
        engine = DeduplicationEngine(chunk_size=4096)

        # Create content smaller than 3 * 4096
        ...
        with tempfile.NamedTemporaryFile(
            ...
            # Spy on FullHasher.compute
            with patch.object(
                engine.full_hasher, "compute", side_effect=engine.full_hasher.compute
            ) as mock_full_compute:

                # First file - unique size, deferred
                engine.check_duplicate(Path(f1.name))
                assert mock_full_compute.call_count == 0

                # Second file - size collision, partial hash collision
                # Should detect duplicate without calling full_hasher.compute
                result = engine.check_duplicate(Path(f2.name))
