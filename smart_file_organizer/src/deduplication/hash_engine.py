...
class DeduplicationEngine:
    ...
    def check_duplicate(self, file_path: Path) -> HashResult:
        ...
        # Optimization: If file is small enough, partial hash IS the full hash.
        ...
        for candidate in candidates:
            if candidate not in self._path_to_full_hash:
                ...
                try:
                    if is_small_file:
                        # If current file is small, candidates (same size) are also small.
                        # Partial hash IS full hash.
