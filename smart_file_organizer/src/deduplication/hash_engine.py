...
class DeduplicationEngine:
    ...
    def check_duplicate(self, file_path: Path) -> HashResult:
        ...
        # Avoid re-reading the file to compute full hash.
        ...
        for candidate in candidates:
            if candidate not in self._path_to_full_hash:
                ...
                try:
                    # Apply same optimization for candidates (they have same size)
                    if file_size <= self.partial_hasher.chunk_size * 3:
                        # We know partial hash exists because we are iterating candidates from _partial_hash_index
