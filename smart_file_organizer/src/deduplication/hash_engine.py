...
class PartialHasher:
    ...
    def is_calculating_full_hash(self, file_size: int) -> bool:
        """Check if partial hash calculation will cover the entire file.

        Args:
            file_size: Size of the file in bytes.

        Returns:
            True if partial hash is effectively a full hash.
        """
...
class DeduplicationEngine:
    ...
    def check_duplicate(self, file_path: Path) -> HashResult:
        ...
        # Optimization: If partial hash was actually a full hash, reuse it
        ...
        if file_path in self._path_to_full_hash:
            full_hash = self._path_to_full_hash[file_path]
        else:
