...
class PartialHasher:
    ...
    def is_calculating_full_hash(self, file_size: int) -> bool:
        """Check if partial hash calculation covers the entire file.

        Args:
            file_size: Size of the file in bytes.

        Returns:
            True if file is small enough that partial hash is full hash.
        """
        ...
        for candidate in candidates:
            if candidate not in self._path_to_full_hash:
                ...
                try:
                    ...
                    if self.partial_hasher.is_calculating_full_hash(cand_size):
                        # Optimization: candidate is small, reuse partial hash
                ...
                except (OSError, DeduplicationError):
                    # If we can't read the candidate anymore, ignore it
                    continue
