...
class DeduplicationEngine:
    ...
    def check_duplicate(self, file_path: Path) -> HashResult:
        ...
        # Optimization: For small files, the partial hash IS the full hash.
        ...
        for candidate in candidates:
            if candidate not in self._path_to_full_hash:
                ...
                try:
                    # Apply same optimization for candidates
                    # Since they have same partial hash, they have similar structure
                    # But we should rely on their size (which we know matches file_size if in this bucket)
                    if file_size <= self.partial_hasher.chunk_size * 3:
