...
class WatcherConfig:
    ...
    watch_directories: List[Path] = field(
        default_factory=lambda: [Path.home() / "Downloads", Path.home() / "Desktop"]
    )
...
class ClassificationConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ClassificationConfig":
        ...
        return cls(
            ...
            fallback_to_zero_shot=data.get(
                "fallback_to_zero_shot", cls.fallback_to_zero_shot
            ),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SecurityConfig":
        ...
        return cls(
            ...
            argon2_memory_cost=int(
                data.get("argon2_memory_cost", cls.argon2_memory_cost)
            ),
            argon2_time_cost=int(data.get("argon2_time_cost", cls.argon2_time_cost)),
            argon2_parallelism=int(
                data.get("argon2_parallelism", cls.argon2_parallelism)
            ),
            secure_delete_passes=int(
                data.get("secure_delete_passes", cls.secure_delete_passes)
            ),
        )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DeduplicationConfig":
        ...
        return cls(
            ...
            perceptual_hash_threshold=int(
                data.get("perceptual_hash_threshold", cls.perceptual_hash_threshold)
            ),
...
class OrganizationConfig:
    ...
    vault_directory: Path = field(
        default_factory=lambda: Path.home() / "Organized" / "Vault"
    )
    quarantine_directory: Path = field(
        default_factory=lambda: Path.home() / "Organized" / ".quarantine"
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "OrganizationConfig":
        ...
        quarantine_dir = Path(
            data.get("quarantine_directory", "~/Organized/.quarantine")
        ).expanduser()
