...
class AESEncryptor:
    ...
    def encrypt_bytes(
        self, data: bytes, key: bytes, associated_data: Optional[bytes] = None
    ) -> bytes:
    ...
    def decrypt_bytes(
        self, encrypted_data: bytes, key: bytes, associated_data: Optional[bytes] = None
    ) -> bytes:
    ...
    def encrypt_file(self, input_path: Path, output_path: Path, key: bytes) -> Path:
        """Encrypt a file.
...
class SecureArchiver:
    ...
    def create_single_file_archive(self, file_path: Path, password: str) -> Path:
    ...
    def extract_archive(
        self, archive_path: Path, password: str, dest_dir: Path
    ) -> List[Path]:
        ...
        try:
            ...
            logger.info(f"Extracted {len(extracted)} files from {archive_path.name}")
            return extracted
