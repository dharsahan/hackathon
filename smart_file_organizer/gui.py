...
class StatsCard(ctk.CTkFrame):
    ...
    def __init__(self, master, title: str, value: str, icon: str = "📊", **kwargs):
        ...
        self.title_label = ctk.CTkLabel(
            self, text=title, font=ctk.CTkFont(size=12), text_color=("gray50", "gray60")
        )
...
class SmartOrganizerGUI(ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        sidebar = ctk.CTkFrame(
            self, width=200, corner_radius=0, fg_color=("#f5f5f5", "#1a1a1a")
        )
        ...
        ctk.CTkLabel(logo_frame, text="🗂️", font=ctk.CTkFont(size=36)).pack()
    ...
    def _show_dashboard(self):
        ...
        cards_data = [
            ...
            (
                "Total Size",
                f"{stats.get('total_size_bytes', 0) / 1024 / 1024:.1f} MB",
                "💾",
            ),
        ]
        ...
        activity_scroll = ctk.CTkScrollableFrame(
            self.content_frame, corner_radius=10, fg_color=("#fafafa", "#151515")
        )
    ...
    def _show_rules(self):
        ...
        ctk.CTkLabel(
            header, text="Custom Rules", font=ctk.CTkFont(size=24, weight="bold")
        ).pack(side="left")
        ...
        if rules:
            for rule in sorted(rules, key=lambda r: -r.priority):
                item = RuleItem(
                    rules_scroll, rule, self._toggle_rule, self._delete_rule
                )
                item.pack(fill="x", pady=3, padx=5)
        else:
            ctk.CTkLabel(
    ...
    def _add_setting_section(self, parent, title: str, items: list):
        ...
        section = ctk.CTkFrame(
            parent, fg_color=("#ffffff", "#1f1f1f"), corner_radius=10
        )
    ...
    def _show_add_rule_dialog(self):
        ...
        ctk.CTkButton(
            dialog, text="Save Rule", width=200, corner_radius=8, command=save_rule
        ).pack(pady=30)
